...
class ASDADataset:
    ...
    def __init__(self, cfg, dataset, domain, data_dir, num_classes, batch_size=128, num_workers=4, transforms=None):
        ...
        self.build_dsets(transforms, cfg)

    def get_num_classes(self):
        return self.num_classes

    def get_dsets(self):
        return self.train_dataset, self.query_dataset, self.valid_dataset, self.test_dataset, self.strong_dataset

    def build_dsets(self, transforms=None, cfg=None):
        assert transforms is not None

        train_list = open(os.path.join(self.data_dir, self.dataset, self.domain+".txt")).readlines()
        ...
        self.strong_dataset = ImageList(test_list, root=self.data_dir, transform=TransformFixMatch(cfg))
